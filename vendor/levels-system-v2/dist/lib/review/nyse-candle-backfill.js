...
export const NYSE_DEFAULT_TIMEFRAMES = ["daily", "4h", "5m"];
export const NYSE_LOOKBACKS = {
    daily: 220,
    "4h": 180,
    "5m": 240,
    "1m": 390,
...
export function buildNyseCleanUniverse(snapshot) {
    return snapshot.rows
        .filter((row) => row.isLikelyCommonEquity)
        .sort((left, right) => left.marketCapBucket.localeCompare(right.marketCapBucket) ||
...
export function buildNyseCandleBackfillPlan(params) {
    ...
    const tasks = buildNyseCleanUniverse(params.snapshot).map((row) => {
...
export function formatNyseCandleBackfillPlan(plan) {
    const lines = [
